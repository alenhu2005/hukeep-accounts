...
function tripCardHTML(t, listIndex = 0) {
  ...
  const closedBadge = t._closed
    ...
    : '';
  return `<div class="trip-card${t._closed ? ' is-voided' : ''}" style="--trip-i:${listIndex}" role="button" tabindex="0" aria-label="查看行程 ${esc(t.name)}" onclick='navigate("tripDetail",${jq(t.id)})' onkeydown='if(event.key==="Enter"||event.key===" "){event.preventDefault();navigate("tripDetail",${jq(t.id)})}'>
