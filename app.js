...
/** 每人依分攤應負擔金額加總（不含已撤回） */
function computeMemberShareTotals(members, expenses) {
  const out = {};
  members.forEach(m => { out[m] = 0; });
  for (const e of expenses) {
    if (e._voided) continue;
...
/** 依日小計（僅有效消費） */
function computeTripDaySubtotals(expenses) {
  const byDay = {};
  for (const e of expenses) {
    if (e._voided) continue;
    const d = e.date || '（無日期）';
...
function renderTripStatsCard(members, expenses) {
  ...
  const shareBlock = `
    <div class="trip-stats-section">
      ...
      <div class="payer-stats-list">
        ${shareRows.map(name => `<div class="payer-stats-row payer-stats-row-plain">
          <span class="payer-stats-name">${esc(name)}</span>
...
function renderTripDetail() {
  ...
  document.getElementById('detail-count').textContent = voidExpN > 0
    ? `有效 ${activeExp.length} 筆 · 共 ${expenses.length} 筆`
    : `${activeExp.length} 筆`;
